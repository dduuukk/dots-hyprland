...
-- # MOVED: was SUPER+K (now used for vim focus-up), then SUPER+SHIFT+K (collided with vim move-up). OSK is now CTRL+ALT+K.
hl.bind("CTRL + ALT + K", hl.dsp.global("quickshell:oskToggle"), { description = "Shell: Toggle on-screen keyboard" })
...
-- # MOVED: was SUPER+J (now used for vim focus-down), then SUPER+SHIFT+J (collided with vim move-down). Bar toggle is now CTRL+ALT+J.
hl.bind("CTRL + ALT + J", hl.dsp.global("quickshell:barToggle"), { description = "Shell: Toggle bar" })
...
--#/# bind = SUPER + ←/↑/→/↓,, -- Focus in direction (arrow keys)
for i = 1, 4 do
...
--#/# bind = SUPER + h/j/k/l,, -- Focus in direction (vim keys)
for i = 1, 4 do
	local vimkey = { "h", "j", "k", "l" }
	local focusdir = { "l", "d", "u", "r" }
	local descdir = { "left", "down", "up", "right" }
	hl.bind(
		"SUPER + " .. vimkey[i],
		hl.dsp.focus({ direction = focusdir[i] }),
		{ description = "Window: Focus " .. descdir[i] .. " (vim)" }
	)
end
for i = 1, 2 do
...
--#/# bind = SUPER + SHIFT, ←/↑/→/↓,, -- Move in direction (arrow keys)
for i = 1, 4 do
	local arrowkey = { "Left", "Right", "Up", "Down" }
	local focusdir = { "l", "r", "u", "d" }
	hl.bind(
...
--#/# bind = SUPER + SHIFT, h/j/k/l,, -- Move in direction (vim keys)
for i = 1, 4 do
	local vimkey = { "h", "j", "k", "l" }
	local focusdir = { "l", "d", "u", "r" }
	local descdir = { "left", "down", "up", "right" }
	hl.bind(
		...
		hl.dsp.window.move({ direction = focusdir[i] }),
		{ description = "Window: Move " .. descdir[i] .. " (vim)" }
	)
end

--# Resize active window with vim keys (CTRL+SUPER + h/j/k/l, 40px steps)
for i = 1, 4 do
	local vimkey = { "h", "j", "k", "l" }
	local dx = { -40, 0, 0, 40 }
	local dy = { 0, 40, -40, 0 }
	local descdir = { "shrink width", "grow height", "shrink height", "grow width" }
	hl.bind(
		"CTRL + SUPER + " .. vimkey[i],
		hl.dsp.window.resize({ x = dx[i], y = dy[i], relative = true }),
		{ repeating = true, description = "Window: Resize " .. descdir[i] .. " (vim)" }
	)
end
...
--#/# bind = CTRL+SUPER+SHIFT, h/l,, -- Focus workspace left/right (vim)
hl.bind(
	"CTRL + SUPER + SHIFT + h",
...
--##! Session
-- # MOVED: was SUPER+L (now used for vim focus-right). Lock is now CTRL+ALT+L.
-- # (CTRL+SUPER+L would collide with the new vim resize binds.)
hl.bind("CTRL + ALT + L", hl.dsp.exec_cmd("loginctl lock-session"), { description = "Session: Lock" })
-- # MOVED: was SUPER+SHIFT+L (now used for vim move-window-right). Sleep is now CTRL+ALT+SHIFT+L.
hl.bind(
	"CTRL + ALT + SHIFT + L",
